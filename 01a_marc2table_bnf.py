...
def get_ark(record):
    result = get_mc_values(record, ["003"])
...
dir_path = "/media/fpichenot/Samsung_T5/bnf_data/SRU"
#dir_path = "data"
sub_dir_marc = "marc/marc2csv"
sub_dir_csv = "csv"
folder = join(dir_path, sub_dir_marc)
for file in get_files(folder):
    #if file[-4:] == 'UTF8':
    marc_file = join(folder, file)
    csv_file = join(dir_path, sub_dir_csv, f"{file[:-4]}.csv")
    print(file)
    ...
    with open(marc_file, 'rb') as fh:
        ...
        for record in list(wrapper(reader)):
            ...
            if record:
                ...
                metadata['publication_place'] = get_publication_place(record)
                ...
                metadata['bnf_authnumbers'] = get_links(record)
...
df_all = pd.DataFrame()
for file in get_files(join(dir_path, sub_dir_csv)):
    df = pd.read_csv(join(dir_path, sub_dir_csv, file))
    df_all = pd.concat([df_all, df])

df_all.to_csv(join(dir_path, sub_dir_csv, 'bnf_mon.csv.gz'), index=False)
